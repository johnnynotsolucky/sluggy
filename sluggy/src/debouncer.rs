//! Copied from https://github.com/notify-rs/notify/tree/main/notify-debouncer-mini
...
pub use notify;
use notify::{Error as NotifyError, ErrorKind, Event, RecommendedWatcher, Watcher};
...
pub type DebounceEventResult = std::result::Result<Vec<DebouncedEvent>, Vec<NotifyError>>;
...
impl DebounceDataInner {
	...
	#[inline]
	pub fn add_error(&mut self, e: notify::Error) {
		self.e.push(e);
	}
...
pub fn new_debouncer_opt<F: DebounceEventHandler, T: Watcher>(
	...
	let tick = match tick_rate {
		Some(v) => {
			if v > timeout {
				return Err(NotifyError::new(ErrorKind::Generic(format!(
		...
		None => timeout.checked_div(tick_div).ok_or_else(|| {
			NotifyError::new(ErrorKind::Generic(format!(
	...
	let watcher = T::new(
		#[inline]
		move |e: std::result::Result<Event, NotifyError>| {
...
///
/// Timeout is the amount of time after which a debounced event is emitted or a continuous event is
/// send, if there still are events incoming for the specific path.
///
/// If tick_rate is None, notify will select a tick rate that is less than the provided timeout.
